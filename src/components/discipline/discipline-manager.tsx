    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Suivi des présences et de la discipline de l'établissement.</p>
        </div>
      ...
      <Tabs defaultValue="absences" className="w-full">
        <TabsList className="grid w-full grid-cols-2 mb-8 bg-slate-100 p-1 rounded-2xl h-14">
          ...
          <TabsTrigger value="discipline" className="rounded-xl data-[state=active]:bg-white data-[state=active]:shadow-sm gap-2">
            <ShieldAlert className="w-4 h-4" /> Conseil de Discipline
          </TabsTrigger>
        </TabsList>
            ...
            <Card className="lg:col-span-2 border-none shadow-md overflow-hidden">
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {absences.filter(a => students.find(s => s.id === a.eleveId)?.classLevel === selectedClass).map((a) => (
                      <TableRow key={a.absenceId}>
                        <TableCell className="pl-6 font-bold">
                          {students.find(s => s.id === a.eleveId)?.name || a.eleveId}
                          <p className="text-[10px] font-mono text-muted-foreground">{a.eleveId}</p>
                        </TableCell>
                    ...
                    {absences.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={4} className="text-center py-12 text-muted-foreground italic">Aucune absence enregistrée pour cette classe.</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
            ...
            <Card className="lg:col-span-2 border-none shadow-md overflow-hidden">
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {incidents.filter(i => students.find(s => s.id === i.eleveId)?.classLevel === selectedClass).map((i) => (
                      <TableRow key={i.incidentId}>
                        <TableCell className="pl-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-slate-800">{students.find(s => s.id === i.eleveId)?.name || i.eleveId}</p>
